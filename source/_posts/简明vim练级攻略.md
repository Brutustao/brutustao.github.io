...
![Word moves example](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/word_moves.jpg)
...
##### [](#在当前行上移动光标-0-f-F-t-T)在当前行上移动光标: `0` `^` `$` `f` `F` `t` `T` `,` `;`
...
![Line moves](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/line_moves.jpg)


还有一个很有用的命令是 `dt"` → 删除所有的内容，直到遇到双引号—— `"。`


##### [](#区域选择-lt-action-gt-a-lt-object-gt-或-lt-action-gt-i-lt-object-gt)区域选择 `<action>a<object>` 或 `<action>i<object>`
...
![Text objects selection](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/textobjects.png)


##### [](#块操作-lt-C-v-gt)块操作: `<C-v>`
...
![Rectangular blocks](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/completion.gif)


在Windows下的vim，你需要使用 `<C-q>` 而不是 `<C-v>` ，`<C-v>` 是拷贝剪贴板。


##### [](#自动提示：-lt-C-n-gt-和-lt-C-p-gt)自动提示： `<C-n>` 和 `<C-p>`

在 Insert 模式下，你可以输入一个词的开头，然后按 `<C-p>或是<C-n>，自动补齐功能就出现了……`


``![Completion](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/completion.gif)
...
![Macros](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/macros.gif)


##### [](#可视化选择：-v-V-lt-C-v-gt)可视化选择： `v`,`V`,`<C-v>`
...
![Autoindent](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/autoindent.gif)
...
![Append to many lines](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/append-to-many-lines.gif)


##### [](#分屏-split-和-vsplit)分屏: `:split` 和 `vsplit`.
...
![Split](https://biantaopicgo.oss-cn-beijing.aliyuncs.com/split.gif)
